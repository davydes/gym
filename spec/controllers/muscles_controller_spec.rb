...
RSpec.describe MusclesController, type: :controller do
  before (:context) do
  ...
  describe "DELETE destroy" do
    describe "when user logged in" do
      ...
      it "should destroy muscle" do
        expect{delete :destroy, :id => @muscle.id}.to change{Muscle.count}.by(-1)
      end

      it "should redirect to muscles_path" do
        delete :destroy, :id => @muscle.id
        expect(response).to redirect_to(muscles_path)
      end
    end
  end
